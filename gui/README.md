...
- **BUG 修复**：Markdown 解析器死循环（4 反引号代码块 OOM）
- **BUG 修复**：中文 slug 文章加载失败（API 双重编码）
- **BUG 修复**：编辑器加载大文章时主线程同步阻塞（改为防抖渲染）
- **新增**：`getTotalViewCount()` / `getAllViewCounts()` / `formatViewCount()` 浏览量聚合工具
- **新增**：DashboardView + FilesView 5s 定时轮询，同标签页浏览量实时同步
- **优化**：总浏览量计算从逐个调用改为单次 `load()` 聚合
- **优化**：所有组件新增 `onBeforeUnmount` 清理定时器，防止内存泄漏

### v0.1.0 (2026-06-24)

- 初始发布，五大核心模块：工作台、Markdown 编辑器、文件管理、一键部署、本地预览
- 自研零依赖 Markdown 渲染器（161 行）
- Pinia 状态管理 + SSE 实时日志
- 部署/预览四态机（running / success / error / stopped）
